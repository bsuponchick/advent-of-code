...
const execute = () => {
    if (debug) {
        console.log(`There are ${seedRanges.length} seed ranges`);
    ...
    let sumOfSeeds = seedRanges.reduce((sum, seedRange) => sum + seedRange[1] - seedRange[0] + 1, 0);
    console.log(`Sum of seeds: ${sumOfSeeds}`);

    seedRanges.forEach((seedRange) => {
        for (let i = seedRange[0]; i <= seedRange[1]; i++) {
            const location = getDestinationForSource(humidityToLocationRanges, getDestinationForSource(temperatureToHumidityRanges, getDestinationForSource(lightToTemperatureRanges, getDestinationForSource(waterToLightRanges, getDestinationForSource(fertilizerToWaterRanges, getDestinationForSource(soilToFertilizerRanges, getDestinationForSource(seedToSoilRanges, i)))))));
...
const parseSeeds = (line: string) => {
    line.split(' ').forEach((seed, index, array) => {
        if (index % 2 === 0) {            
            const start = parseInt(array[index], 10);
            const range = parseInt(array[index + 1], 10);
