...
export const extrapolateNextValue = (sequence: number[]): number => {
    ...
    sequences.reverse().forEach((sequence, index) => {
        const nextValue = determineNextValue(sequence, index === 0 ? 0 : sequences[index - 1][sequences[index - 1].length - 1]);
        sequence.push(nextValue);
    });

    return sequences[sequences.length - 1][sequences[sequences.length - 1].length - 1];
};