...
export class CephalopodCalculator {
    ...
    public static executeCommandsAndAddResults(commands: Command[]): number {
        ...
        for (const command of commands) {
            // Need to conver columns of numbers properly
            const stringParts: string[][] = [];
            
            for (const value of command.values) {
                const valueAsString = `${value}`;
                const valueAsStringArray = valueAsString.split('');
                stringParts.push(valueAsStringArray);
            }

            console.log(`stringParts: ${JSON.stringify(stringParts)}`);
