...
export class Grid {
    ...
    getTilesWithForkliftAccess(): Tile[] {
        const tilesWithForkliftAccess: Tile[] = [];
        this.tiles.forEach((row) => {
            row.forEach((tile) => {
                if (tile.canForkliftAccess()) {
                    tilesWithForkliftAccess.push(tile);
                }
            });
        });
        return tilesWithForkliftAccess;
    }

    countAndRemoveTilesWithForkliftAccess(): number {
        let countOfTilesWithForkliftAccess = 0;
        const tilesWithForkliftAccess = this.getTilesWithForkliftAccess();
        tilesWithForkliftAccess.forEach((tile) => {
            tile.type = '.';
            countOfTilesWithForkliftAccess++;
        });
        return countOfTilesWithForkliftAccess;
    ...
    determineMaxNumberOfTilesThatBeRemoved(): number {
        let totalNumberOfTilesRemoved = 0;

        while (this.countTilesWithForkliftAccess() > 0) {
            this.printWithForkliftAccess();
            console.log(`Removing tiles with forklift access...`);
            totalNumberOfTilesRemoved += this.countAndRemoveTilesWithForkliftAccess();
        }

        return totalNumberOfTilesRemoved;
    }
