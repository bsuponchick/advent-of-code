class Monkey {
    ...
    getBored = (value) => {
        ...
        console.log(`Monkey gets bored with the item.  Worry level is divided by 3 to ${newValue}`);
        return newValue;
    }

    throw = (value, target) => {
        console.log(`Item with worry level of ${value} is thrown to Monkey ${target}`);
        this.items.shift();
...
for (let round = 0; round < 20; round++) {
    ...
    console.log(`After ${round + 1} rounds, the monkeys are holding items with these worry levels:`);
    monkeys.forEach((monkey) => {
        monkey.report();
    });
}
