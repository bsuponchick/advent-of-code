        ...
        if (nodes.length === 1) {
            ...
            for (let i = 0; i < nodes.length; i++) {
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    nodeA.addAntiNode(nodeA.symbol);
                    nodeB.addAntiNode(nodeB.symbol);

                    const potentialAntiNodes = nodeA.getAntiNodeCoordinates(nodeB, { x: map[0].length, y: map.length });
