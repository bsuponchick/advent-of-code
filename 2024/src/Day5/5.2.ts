...
let invalidUpdates: PotentialUpdate[] = [];
...
const execute = () => {
    potentialUpdates.forEach((potentialUpdate) => {
        if (potentialUpdate.determineIfValid(rules)) {
            ...
            invalidUpdates.push(potentialUpdate);
        }
    });

    invalidUpdates.forEach((invalidUpdate) => {
        invalidUpdate.reorderPageNumbers(rules); 
    });

    const middlePageNumbers = invalidUpdates.map((invalidUpdate) => invalidUpdate.getMiddlePageNumber());
